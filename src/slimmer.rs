...
/// "#.to_string();
///
/// let cleaned_html = slim_html(html_content).unwrap();
...
pub fn slim(html_content: &str) -> Result<String> {
	...
	let content = String::from_utf8(output).map_err(|err| format!("html5ever serialization non utf8. {err}"))?;
...
fn remove_non_content_tags(handle: &Handle) -> Result<()> {
	...
	for &index in indices_to_remove.iter().rev() {
		handle
			.children
			.try_borrow_mut()
			.map_err(|err| format!("html5ever child already borrowed. {err}"))?
			.remove(index);
	}
